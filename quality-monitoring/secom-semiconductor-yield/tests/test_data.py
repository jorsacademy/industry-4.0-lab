...
def test_loader_parses_quoted_labels_and_timestamp(tmp_path: Path):
...
def test_loader_also_accepts_unquoted_timestamps(tmp_path: Path):
    data = tmp_path / "secom.data"
    labels = tmp_path / "secom_labels.data"
    data.write_text("1 2\n3 4\n", encoding="utf-8")
    labels.write_text("-1 19/07/2008 11:55:00\n1 19/07/2008 12:00:00\n", encoding="utf-8")
    df = load_secom(data, labels)
    assert df["raw_label"].tolist() == [-1, 1]
    assert df["timestamp"].notna().all()
...
def test_chronological_blocks_have_no_time_overlap():
    ...
    for i in range(40):
        rows.append({
            ...
            "raw_label": 1 if i % 5 == 0 else -1,
            "is_fail": 1 if i % 5 == 0 else 0,
            "source_row": i,
        })
